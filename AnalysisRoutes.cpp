...
void Analysis_Tools::AnalysisRoutes() {
	...
	CROW_ROUTE(AnalysisApp, "/punishment_apply").methods(crow::HTTPMethod::POST)([&](const crow::request& req) {
		...
		//data: {"piloto_punido":"1","campo_temp":"T21E1","situacaoPrincipal":"1","situacaoIntrinseca":"5","reincidencia":false}
	...
	CROW_ROUTE(AnalysisApp, "/<path>")([](const string& filename) {
	...
	AnalysisApp.port(8085).multithreaded().run();
