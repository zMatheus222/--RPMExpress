...
void Exlog(string function, string message) {
	if (Debug_Logs == "true") {
		if((regex_search(function, regex(Exlog_Shows))) || (regex_match(string(Exlog_Shows), regex("All")))) cout << NEGRITO << BRANCO << "[" << function << BRANCO << "] " << PADRAO << message << ENDS;
	}
}

void Exlog(const char* function, const char* service, const char* route_type, const char* message) {
	if (Debug_Logs == "true") {
		if ((regex_search(function, regex(Exlog_Shows))) || (regex_match(string(Exlog_Shows), regex("All")))) cout << NEGRITO << BRANCO << "[" << PRETO << function << PADRAO << BRANCO << "::" << MAGENTA << service << BRANCO << "::" << CIANO << route_type << NEGRITO << BRANCO << "] " << PADRAO << BRANCO << message << ENDS;
	}
}
