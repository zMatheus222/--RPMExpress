...
void MySQL::Connect(string server, string username, string password) {
	...
	try {
		...
		Exlog("INFO", AMARELO + __FUNCTION__, VERDE + "Logado com sucesso no banco de dados MySQL!");
	}
	catch (sql::SQLException e) {
		Exlog("ERROR", AMARELO + __FUNCTION__, VERMELHO + "Não foi possível logar no banco de dados MySQL, Mensagem de erro: " + e.what());
	}
...
vector<string> MySQL::list_schemas(regex filter) {
	...
	try {
		...
		Exlog("INFO", AMARELO + __func__, BRANCO + "Schema list:");
		...
		while (result->next()) {
			...
			if (!regex_search("none", filter) && regex_search(result->getString(1).c_str(),filter)) {
				//Exlog("INFO", AMARELO + __func__, BRANCO + "filter" + CIANO + filter + BRANCO + " | getString(1): " + CIANO + result->getString(1).asStdString());
				schema_list.push_back(result->getString(1));
			}
		...
		Exlog("ERROR", AMARELO + __func__, VERMELHO + "Problema durante a listagem de banco de dados.");
		Exlog("ERROR", AMARELO + __func__, VERMELHO + "Erro SQL: " + e.what());
		Exlog("ERROR", AMARELO + __func__, VERMELHO + "Código do erro: " + to_string(e.getErrorCode()));
...
vector<string> MySQL::list_tables(string schema, regex filter) {
	...
	Exlog("INFO", AMARELO + __func__, BRANCO + "to_search: " + CIANO + schema);
	...
	try {
		...
		Exlog("INFO", AMARELO + __func__, BRANCO + "getSchema on: " + CIANO + con->getSchema().asStdString());
		...
		Exlog("INFO", AMARELO + __func__, "Tablelist:");
		while (result->next()) {
		...
		Exlog("INFO", AMARELO + __func__, VERDE + "Listagem de tabelas concluida.");

	}
	catch (sql::SQLException& e) {

		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "Algum problema na listagem das tables do schema: " + schema + ", Mensagem de erro : " + e.what());
...
void MySQL::create_schema(string db_name) {
	...
	try {
		...
		Exlog("INFO", MAGENTA + __func__, BRANCO + "Created Schema: " + CIANO + db_name);
	}
	catch (sql::SQLException& e) {
		
		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "Não foi possível criar o banco de dados. Mensagem de erro: " + e.what());
		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "Erro SQL: " + e.what());
		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "Código de erro: " + to_string(e.getErrorCode()));
		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "SQLState: " + e.getSQLState());
...
void MySQL::create_table(string schema, string table_name, bool drop_if_exists, string table_index) {
	...
	Exlog("INFO", MAGENTA + __func__, BRANCO + "table_index: " + CIANO + table_index);
	...
	try {
		...
		stmt = con->createStatement();
		Exlog("INFO", MAGENTA + __func__, VERDE + "Created Statement (stmt)");

		if (drop_if_exists == true) {
			stmt->execute("DROP TABLE IF EXISTS " + table_name);
			Exlog("INFO", MAGENTA + __func__, VERDE + "Dropped table: " + table_name);
		}

		stmt->execute("CREATE TABLE IF NOT EXISTS " + table_name + " (" + table_index + ");");
		Exlog("INFO", MAGENTA + __func__, VERDE + "Created table: " + table_name);

	}
	catch (sql::SQLException& e) {

		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "Problema durante a criacao da tabela: " + table_name);
		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "Erro SQL: " + e.what());
		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "Código de erro: " + to_string(e.getErrorCode()));
		Exlog("ERROR", MAGENTA + __func__, VERMELHO + "SQLState: " + e.getSQLState());
...
int MySQL::findDriverOnTable(string& needed_table, string& piloto_acusado) {
	...
	try {
		...
		if (regex_match(piloto_acusado, regex("^S\\d{10,25}$"))) {
			...
			Exlog("INFO", AMARELO + __func__, BRANCO + "SteamGuid::Preparando Statment (pstmt): " + CIANO + QueryFindDriverOnTableSTEAMGUID(needed_table, piloto_acusado));

			result = pstmt->executeQuery();

			Exlog("INFO", AMARELO + __func__, BRANCO + "Realizando SELECT na table " + needed_table + " da temporada atual.");

			while (result->next()) {
				playerKeyId = result->getInt(1);
				Exlog("INFO", AMARELO + __func__, BRANCO + "PlayerId: " + CIANO + to_string(playerKeyId));
			}
			...
			Exlog("INFO", AMARELO + __func__, BRANCO + "lastName::Preparando Statment (pstmt): " + CIANO + QueryFindDriverOnTable(needed_table, piloto_acusado));
